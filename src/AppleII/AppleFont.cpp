...
// backbuffer ignorado: escrevemos direto no framebuffer da VGA (RAM interna).
// A assinatura foi mantida para nao mexer no AppleFont.h.
void AppleFont::RenderFont(Color* /*backbuffer*/, int fontnum, int posx, int posy, bool inv)
{
	// verde do texto = Color(0, 0xFF, 0) -> RGB222 (0,3,0) -> indice 12
	const uint8_t rawOn  = _rawLUT[(0 << 4) | (3 << 2) | 0];
	const uint8_t rawOff = _rawLUT[0];   // preto
	...
	for (int y = 0; y < FONT_Y; y++)
		for (int x = 0; x < FONT_X; x++)
		{
			...
			fbPoint(posx + x, posy + y, c == 1 ? rawOn : rawOff);
		}
}
